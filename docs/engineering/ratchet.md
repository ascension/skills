...
`ratchet` is a reach-for-it-anytime standalone that compounds: every click makes the codebase safer and more legible for the next agent and the next contributor. It pairs naturally with [diagnosing-bugs](https://aihero.dev/skills-diagnosing-bugs) — after a hard bug is fixed, `ratchet` asks whether the whole class can be locked out — and with [code-review](https://aihero.dev/skills-code-review), whose repeated findings are exactly the classes worth encoding. When you're unsure which skill fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
