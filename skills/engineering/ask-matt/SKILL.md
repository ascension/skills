...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`/to-spec`** (turn the thread into a spec), then **`/to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. On a local tracker that's one file per ticket under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any ticket whose blockers are done can be grabbed — kick off **`/implement`** per ticket, **`/clear`ing context between each one**. Each ticket is self-contained, so the last one's context is disposable. When you want that approved graph run AFK — isolated sessions, one draft PR, and a bounded review/fix loop — **`/orchestrate-prd`** is the coordinator.
   - **No** → **`/implement`** right here, in the same context window.

   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point. When the session has opened a PR, **`/retro`** looks back for **stops** — every place the agent had to wait for a human — and encodes each at the highest rung that holds it, so the next session runs through the same constraints without interruption.
...
- **`/retro`** — after a session that shipped or opened a PR, find every **stop** (a question, a correction, a pause, missing-repo context, a human review comment) and encode the class at the highest rung that holds it: architecture or data structures, then a lint or test, then a skill or rule, then human review as a source of feedback to promote. User-invoked. It *finds* the classes across a session and a PR; **`/ratchet`** encodes one class you already have.
- **`/ratchet`** — when the same fix, correction, or busywork shows up twice, encode the whole **class** as infrastructure — a type, lint rule, test, CI check, hook, or a CLAUDE.md/REVIEW.md rule co-located with the code it governs — so it can never come back. Also its proactive mode: audit an area for documentation gaps (questions agents had to ask, conventions written nowhere, rules at the wrong altitude) until an agent could work there with **zero additional context**. Model-invoked: the agent also reaches for it on "make sure this never happens again". Natural follow-on from `/diagnosing-bugs` (lock out the class after the fix), from repeated `/code-review` findings, and from `/retro` (encode each stop it ranked onto a mechanical or prose rung).
...
- **`/orchestrate-prd`** — after `/to-tickets` has an approved graph, coordinate isolated implementation sessions, publish one draft PR, and close a bounded review/fix loop. Workers and coordinator rollover go through **`/dispatch`**. Fork-local and personal rather than part of the promoted plugin.
- **`/dispatch`** — spawn a fresh session through a named or detected adapter (Herdr, Buzz, native, Pi), in a **space** matched by issue, branch, or short work description (create one if none match). Unnamed stays on this host. Approaching the smart zone rolls to a new agent; it does not write a travel file. That file is still **`/handoff`**. Model-invoked. Fork-local and personal rather than part of the promoted plugin.
